...
object RequestHandlers {
    fun getAssertion(args: JSONArray, nfc: NFCDiscoveryDispatcher, dispatch: ResultDispatcher) {
        ...
        nfc.useDeviceConnection(dispatch) ctx@ { connection ->
            ...
            dispatch.sendStatusResult(StatusCodes.SignalProgressUpdate, 0f / 3f)
            ...
            dispatch.sendStatusResult(StatusCodes.SignalProgressUpdate, 1f / 3f)
            ...
            if(credentials.size > 1) {
                dispatch.sendStatusResult(StatusCodes.SuccessUserChoiceRequired, Fido.encodeCredentialList(credentials))
                return@ctx
            }

            dispatch.sendStatusResult(StatusCodes.SignalProgressUpdate, 2f / 3f)
            ...
            dispatch.sendStatusResult(StatusCodes.SignalProgressUpdate, 3f / 3f)
            val result = Fido.encodePublicKeyCredential(assertion, credentials.first(), clientData)

            dispatch.sendStatusResult(StatusCodes.Success, result)
        }
    }
}